...
不想一檔一檔選時，選好**交易日**（**代碼下拉可不管**），按 **批次偵測（全部）**：

- 對當天**所有有錄到 Tick 的股票**逐一跑偵測並落地（等同 `DetectionService.run_all`）。
- 所有股票的事件**彙整成一張清單**，依時間排序，靠最左的 **代碼** 欄區分是哪一檔。
- 完成後訊息回報：`✓ 批次完成：N 檔、K 檔有觸發、共 M 事件`。
- 執行期間「跑偵測」與「批次偵測」兩個鈕都會暫時停用，避免重複觸發。

適合盤後「先掃一遍今天哪些股票有發動事件」，再針對有料的個股回單檔細看。
